...
export class ClientEmployeesComponent implements OnInit {
  ...
  constructor(private router : Router,private clientEmployeesService : ClientEmployeesService,private activatedRoute : ActivatedRoute) {       
       this.sorting="none";
  ...
  getclientEmployees(){    
      this.clientEmployeesService.getClientEmployees(this.page,this.count,this.CliId).subscribe(
         (data)=>{this.mainClientEmployees=data['clientEmployees'];this.clientEmployees=this.mainClientEmployees;
      ...
      this.error="";  
         ...
         (error)=>{this.error="Error fetching client employees"}
       )
  }
